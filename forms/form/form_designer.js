$(document).ready(function(){
	...
	function formDesigner_clickElement(that) {
		...
		if (!$(that).is("#formDesignerToolBtn") && !$(that).parents("#formDesignerToolBtn").length) {
			...
			content.removeAttr("data-current").removeAttr("data-hovered");
	...
	function formDesigner_selectSnippet() {
		...
		$("#formDesignerSnippets a[href=#snippet]").on("click",function(){
			...
			$("#"+popover+" a").on("click",function(){
				...
				$.get("/engine/ajax.php?mode=snippet&form=form&snippet="+snippet+"&formname="+form,function(data){
					...
					var data=$(data);
					if ($(that).hasClass("sAppend")) {$(target).append(data);}
					if ($(that).hasClass("sPrepend")) {$(target).prepend(data);}
					if ($(that).hasClass("sAfter")) {$(target).after(data);}
	...
	function formDesigner_unsetElement() {
		...
		$(document).delegate("*","click",function(event){
			...
			if ((	!$(event.target).parents(".formDesignerEditor").length
				...
				&&	!$(event.target).parents(".sidebar-nav").length
